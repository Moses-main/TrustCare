...
import { FaFileInvoiceDollar, FaCreditCard, FaReceipt, FaSearch } from 'react-icons/fa';
      ...
      <div className="bg-white shadow overflow-hidden sm:rounded-lg">
        ...
        {filteredBills.length === 0 ? (
          <div className="p-8 text-center">
            ...
            <h3 className="mt-2 text-sm font-medium text-gray-900">No {selectedView === 'all' ? 'records' : selectedView} found</h3>
          ...
          <div className="bg-white shadow overflow-hidden sm:rounded-md">
            <ul className="divide-y divide-gray-200">
              {filteredBills.map((bill) => (
                <li key={bill.id} className="px-4 py-4 sm:px-6 hover:bg-gray-50">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        ...
                        <div className="mt-1 text-xs text-gray-500">
                          {bill.type === 'invoice' ? (
                            <>
                              <span>Issued: {format(new Date(bill.date), 'MMM d, yyyy')}</span>
                              {bill.dueDate && (
                                <span className="ml-2">• Due: {format(new Date(bill.dueDate), 'MMM d, yyyy')}</span>
                              )}
                            </>
                          ) : (
                    ...
                    <div className="text-right">
                      <div className={`text-lg font-semibold ${
                        bill.type === 'payment' ? 'text-green-600' : 'text-gray-900'
                      }`}>
                        {bill.type === 'payment' ? '-' : ''}{formatCurrency(bill.amount)}
                      </div>
                      {bill.paymentMethod && (
                      ...
                      <div className="mt-2">
                        {bill.type === 'invoice' && bill.status === 'pending' ? (
                          <button
                            type="button"
