...
export const providersAPI = {
  ...
  setAvailability: (availability) =>
    api.post("/providers/availability", availability),
  getAvailability: (providerId, startDate, endDate) =>
    api.get(
      `/providers/${providerId}/availability?start=${startDate}&end=${endDate}`
    ),

  // Appointments
  getUpcomingAppointments: () => api.get("/providers/me/appointments/upcoming"),
  getPastAppointments: (limit = 10) =>
    api.get(`/providers/me/appointments/past?limit=${limit}`),
  updateAppointmentStatus: (appointmentId, status) =>
    api.put(`/appointments/${appointmentId}/status`, { status }),

  // Patient Records
  getPatientRecords: (patientId) =>
    api.get(`/providers/patients/${patientId}/records`),
  addRecordNote: (recordId, note) =>
    api.post(`/records/${recordId}/notes`, { note }),
};
...
export const recordsAPI = {
  ...
  // Record Sharing
  shareRecord: (recordId, sharedWith) =>
    api.post(`/records/${recordId}/share`, { sharedWith }),
  revokeAccess: (recordId, walletAddress) =>
    api.delete(`/records/${recordId}/share/${walletAddress}`),
